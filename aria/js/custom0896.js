...
// ($(".eventViewModal").data("bs.modal") || {})._isShown
$(function () {
	$.fn.extend({
...
function showAlarmMsg(content, category, isModalToastr) {
	...
	Swal.fire(params).then(function() {
		if (category == 'item-letter') {
			$('.mypage-link.mypage-letter-panel').click();
			return;
		}
		if (category == 'item-qna') {
			$('.mypage-link.mypage-qna-panel').click();
		}
	});
}
...
function showLoginForm() {
	...
	if ($(".loginModal").length > 0) {
		...
		$(".loginModal").siblings(".modal").modal("hide");
		return true;
	}
